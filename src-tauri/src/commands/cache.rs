...
fn ensure_dir(path: &Path) -> anyhow::Result<()> {
    fs::create_dir_all(path).with_context(|| {
        format!(
            "CACHE_DIRECTORY_UNAVAILABLE: 无法创建缓存目录：{}",
            path.display()
        )
    })?;
    ensure_marker(path).with_context(|| {
        format!(
            "CACHE_DIRECTORY_UNAVAILABLE: 缓存目录不可写：{}",
            path.display()
        )
    })?;
    Ok(())
...
fn ensure_marker(path: &Path) -> anyhow::Result<()> {
    ...
    {
        Ok(mut file) => file
            .write_all(b"RefMind3D persistent image cache\n")
            .map_err(Into::into),
...
fn status(project_id: &str, directory: Option<&str>, calculate_size: bool) -> CacheStatus {
    ...
    if result.is_ok() {
        repair_cache_once(&path);
    }
    CacheStatus {
        directory: path.to_string_lossy().to_string(),
        size_bytes: if result.is_ok() && calculate_size {
            dir_size(&path)
        } else {
            0
        },
        size_calculated: calculate_size && result.is_ok(),
        max_bytes: DEFAULT_LIMIT_BYTES,
        initialized: directory.is_some_and(|value| !value.trim().is_empty()),
...
pub async fn image_cache_status(
    ...
    calculate_size: Option<bool>,
) -> Result<CacheStatus, String> {
    tauri::async_runtime::spawn_blocking(move || {
        status(
            &project_cache_id,
            cache_directory.as_deref(),
            calculate_size.unwrap_or(false),
        )
    })
    .await
...
#[tauri::command]
pub async fn set_image_cache_directory(
    project_cache_id: String,
    directory: String,
) -> Result<CacheStatus, String> {
    tauri::async_runtime::spawn_blocking(move || {
        if directory.trim().is_empty() {
            return Err("请选择缓存目录".to_string());
        }
        let path = PathBuf::from(directory.trim());
        initialize_dir(&path).map_err(|e| e.to_string())?;
        let value = CacheSettingsFile {
            directory: path.to_string_lossy().to_string(),
            max_bytes: DEFAULT_LIMIT_BYTES,
            initialized: true,
        };
    ...
    .await
    .map_err(|e| format!("设置缓存目录失败：{e}"))?
}

#[tauri::command]
pub async fn confirm_default_image_cache_directory(
    project_cache_id: String,
) -> Result<CacheStatus, String> {
    tauri::async_runtime::spawn_blocking(move || {
...
pub async fn clear_image_cache(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        {
            let path = entry.path();
            if path.file_name().and_then(|v| v.to_str()) == Some(CACHE_MARKER) {
                continue;
            }
            let remove = cutoff
                .map(|c| newest_modified(&path).map(|t| t < c).unwrap_or(true))
                .unwrap_or(true);
            if remove {
...
#[tauri::command]
pub async fn prepare_image_cache(
    project_cache_id: String,
    cache_directory: Option<String>,
    asset: Value,
...
fn prepare_sync_cancelable(
    ...
    if let Ok(raw) = fs::read(&manifest_path) {
        if let Ok(mut manifest) = serde_json::from_slice::<CacheManifest>(&raw) {
            ...
            if manifest.source_size == size
                && manifest.source_modified_ms == modified
    ...
    let bytes = match source.location {
        ...
        SourceLocation::Runtime(url) => {
            runtime_assets::read_resource_url(&url)
                .map_err(|e| anyhow!(e))?
                .0
        }
            ...
            if let Some((size, modified)) = runtime_assets::resource_source_signature(value) {
                if modified > 0 {
                    return Ok(SourceDescriptor {
                        location: SourceLocation::Runtime(value.to_string()),
                        size,
                        modified,
                        hash: 0,
                        extension: extension(value, asset),
                    });
                }
            }
            let (bytes, _, name) =
                runtime_assets::read_resource_url(value).map_err(|e| anyhow!(e))?;
            return Ok(SourceDescriptor {
                size: bytes.len() as u64,
                modified: 0,
                hash: bytes_hash(&bytes),
                extension: extension(&name, asset),
                location: SourceLocation::Memory(bytes),
            });
        }
...
fn extension(name: &str, asset: &Value) -> String {
    Path::new(name)
        .extension()
        .and_then(|v| v.to_str())
        .or_else(|| asset.get("format").and_then(Value::as_str))
        .unwrap_or("png")
        .to_ascii_lowercase()
}
fn decode(bytes: &[u8], ext: &str) -> anyhow::Result<(DynamicImage, Option<Vec<u8>>)> {
    if matches!(ext, "psd" | "psb") {
        ...
        let rgba = image::ImageBuffer::from_raw(psd.width(), psd.height(), psd.rgba())
            .ok_or_else(|| anyhow!("PSD 像素数据无效"))?;
...
fn resize(image: DynamicImage, max: u32) -> DynamicImage {
        ...
        tile_urls: tiles
            ...
            .map(|(index, path)| {
                runtime_assets::register_file_resource(
                    asset_id,
                    &format!("cacheTile:{suffix}:{generation}:{index}"),
                    format!("tile-{index}.png"),
                    "image/png".into(),
                    path.clone(),
                )
            })
...
fn enforce_limit(root: &Path, limit: u64) {
    ...
    let mut units = entries
        ...
        .filter_map(|entry| {
            let path = entry.path();
            if path.file_name().and_then(|value| value.to_str()) == Some(CACHE_MARKER) {
                return None;
            }
...
pub async fn load_model_cover(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let path = root
            .join("model-covers")
            .join(format!("{}.webp", safe(&key)));
        if !path.is_file() {
            return Ok(None);
        }
        Ok(Some(runtime_assets::register_file_resource(
            &format!("model-cover-{}", safe(&key)),
            "preview",
            "cover.webp".into(),
            "image/webp".into(),
            path,
    ...
    .await
    .map_err(|e| format!("读取模型封面失败：{e}"))?
}

#[tauri::command]
pub async fn save_model_cover(
    project_cache_id: String,
    cache_directory: Option<String>,
    key: String,
    data_url: String,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let encoded = data_url
            .split_once(',')
            .map(|(_, data)| data)
            .ok_or_else(|| "模型封面数据无效".to_string())?;
        let bytes = base64::engine::general_purpose::STANDARD
            .decode(encoded)
            .map_err(|e| format!("模型封面解码失败：{e}"))?;
        ...
        Ok(runtime_assets::register_file_resource(
            &format!("model-cover-{}", safe(&key)),
            "preview",
            "cover.webp".into(),
            "image/webp".into(),
...
fn repair_cache_once(root: &Path) {
    ...
    if !roots.lock().unwrap().insert(root.to_path_buf()) {
        return;
    }
        ...
        .take(limit)
    {
        ...
        let valid = manifest.is_some_and(|manifest| {
            path.join(manifest.preview_file).is_file()
                ...
                && manifest
                    .medium_file
                    .is_none_or(|file| path.join(file).is_file())
        });
        if !valid && fs::remove_dir_all(&path).is_ok() {
            removed += 1;
        }
    }
    removed
...
fn now_ms() -> u64 {
    time_ms(SystemTime::now()).unwrap_or(0)
}
...
mod tests {
    ...
    fn concurrent_directory_checks_do_not_share_a_probe_file() {
        ...
        for handle in handles {
            handle
                .join()
                .expect("cache check thread panicked")
                .expect("cache directory check failed");
        }
    ...
    fn default_cache_directory_is_isolated_per_project() {
        assert_ne!(
            project_cache_dir("project-a", None),
            project_cache_dir("project-b", None)
        );
    ...
    fn prepared_image_is_reused_and_invalidated_when_source_changes() {
        ...
        assert!(
            !prepare_sync("project-1", Some(cache.to_string_lossy().as_ref()), &asset)
                .unwrap()
                .cache_hit
        );
    ...
    fn large_image_cache_is_split_into_tiles() {
        let root = env::temp_dir().join(format!(
            "refmind3d-cache-tile-test-{}",
            uuid::Uuid::new_v4()
        ));
        let source = root.join("wide.png");
        let cache = root.join("project-cache");
        fs::create_dir_all(&root).unwrap();
        DynamicImage::new_rgba8(4100, 2).save(&source).unwrap();
